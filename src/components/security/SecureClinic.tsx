      ...
      <div className="grid lg:grid-cols-[300px_1fr] gap-4 print:block">
        ...
        <section className="min-w-0 space-y-4">{!selected ? <p className="bg-white p-6 rounded">Selecione um paciente para consultar cadastro, agenda e autorizações.</p> : <>
          <section className="bg-white p-4 rounded space-y-3 print:hidden"><h2 className="font-bold text-lg">{selected.name}</h2><p>{selected.email} · {selected.phone}</p>
            ...
            <details open={!bundle}><summary>Autorizações do paciente</summary><p className="text-sm">{access?.contactVerified ? 'Contato confirmado.' : 'Primeiro envie um código autorizando o profissional responsável para confirmar o contato.'} O atendimento pode ser autorizado por 8 horas ou por um mês. Um mês conta da confirmação do paciente, sem renovação automática.</p>
              <form className="flex gap-2 flex-wrap mt-2" onSubmit={e => {e.preventDefault(); const v = Object.fromEntries(new FormData(e.currentTarget)); void perform(async () => {setRequest(await secureApi('access.request', selected.id, v)); setNotice('Código enviado ao paciente. Ele também recebeu um link para confirmar e suspender autorizações.');});}}><select aria-label="Profissional a autorizar" className={field} name="targetUid" required defaultValue={selected.ownerUid}>{members.filter(m => m.role !== 'receptionist').map(m => <option key={m.uid} value={m.uid}>{m.name}</option>)}</select><select aria-label="Duração" className={field} name="duration" defaultValue="month"><option value="visit">Somente este atendimento (8 horas)</option><option value="month">Tratamento por um mês — autorizado pelo paciente</option></select><button className={button}>Enviar código ao paciente</button></form>
